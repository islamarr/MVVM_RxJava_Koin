...
class AlbumDetailsViewModelTest {
    ...
    fun setUp() {
        ...
        viewModel =
            AlbumDetailsViewModel(albumDetailsUseCase, getFavoriteUseCase, setFavoriteUseCase)
    }

    @Test
    fun `when load Album details return Loading State as a first result`() = runTest {
    ...
    @Test
    fun `when load Album details return AlbumDetailsData State as a second result`() = runTest {
        val albumParams = AlbumParams(artistName = "artist", albumName = "album")
        val albumEntity = AlbumEntity(trackList = listOf())
        whenever(albumDetailsUseCase.execute(albumParams)).thenReturn(
            AlbumDetailsStates.AlbumDetailsData(
                albumEntity
            )
        )
        whenever(getFavoriteUseCase.execute(albumParams)).thenReturn(
            AlbumDetailsStates.SavedState(
                true
            )
        )

        val actual =
            viewModel.handle(AlbumDetailsActions.AlbumDetailsAction(albumParams)).drop(1).first()
    ...
    @Test
    fun `when load Album details return SavedState State as a third result`() = runTest {
        val albumParams = AlbumParams(artistName = "artist", albumName = "album")
        val albumEntity = AlbumEntity(trackList = listOf())
        whenever(albumDetailsUseCase.execute(albumParams)).thenReturn(
            AlbumDetailsStates.AlbumDetailsData(
        ...
        val actual =
            viewModel.handle(AlbumDetailsActions.AlbumDetailsAction(albumParams)).drop(2).first()
    ...
    @Test
    fun `when dispatch viewModel return SavedState`() = runTest {
        val albumParams = AlbumParams(artistName = "artist", albumName = "album")
        val albumEntity = AlbumEntity(trackList = listOf())
